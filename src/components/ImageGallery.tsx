    ...
    <div>
      ...
      {isOpen.value && (
        <>
          {/* Modal Backdrop */}
          <div
            class="fixed inset-0 flex items-center justify-center bg-charcoal/95 backdrop-blur-sm animate-fadeIn"
          ...
          >
            ...
            <div class="fixed left-1/2 top-4 -translate-x-1/2 rounded-full bg-white/10 px-4 py-2 text-sm font-medium text-white backdrop-blur-md" style="z-index: 10001 !important;">
              {currentIndex.value + 1} / {images.length}
            </div>

            {/* Close Hint */}
            <div class="fixed left-4 top-4 rounded-full bg-white/10 px-3 py-2 text-xs font-medium text-white backdrop-blur-md md:left-6 md:top-6" style="z-index: 10001 !important;">
            ...
            {/* Thumbnail Strip */}
            {images.length > 1 && (
              <div class="absolute bottom-4 left-1/2 flex max-w-full -translate-x-1/2 gap-2 overflow-x-auto rounded-lg bg-white/10 p-3 backdrop-blur-md" style="z-index: 10001 !important;">
                {images.map((image, index) => (
                  <button
                    key={index}
                    onClick$={(e) => {
          ...
          {/* Close Button - Separate fixed element with highest z-index */}
          <button
            onClick$={closeGallery}
            class="fixed right-4 top-4 rounded-full bg-white p-3 text-charcoal shadow-2xl transition-all hover:bg-neutral hover:scale-110 focus:ring-4 focus:ring-white md:right-6 md:top-6 md:p-4"
            style="z-index: 10002 !important;"
            aria-label="Close gallery"
          >
            <svg class="h-7 w-7 md:h-8 md:w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
