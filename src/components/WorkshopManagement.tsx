...
const WorkshopManagement: React.FC = () => {
  ...
  const handleToggleActive = async (section: NavSection) => {
    try {
      ...
      showToast(
        `Section ${(updatedSection as any).active ? 'activated' : 'deactivated'}`,
        'success'
      );
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {workshopSections.map(section => (
          ...
          >
            <div
              className={`w-12 h-12 bg-gradient-to-r ${section.color} rounded-lg flex items-center justify-center text-2xl mb-4`}
            >
              {section.icon}
            </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-4 text-sm text-neutral-500">
                  ...
                  <span>
                    Updated:{' '}
                    {new Date((section as any).updatedAt || Date.now()).toLocaleDateString()}
                  </span>
                </div>
              </div>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-neutral-600 mb-4">
            {searchQuery
              ...
              : 'Get started by creating your first workshop section'}
          </p>
          <button
            onClick={() => {}}
            className="px-4 py-2 bg-accent text-brand rounded-lg hover:bg-accent-dark transition-colors font-medium"
